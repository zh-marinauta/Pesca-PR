...
bd<-read.csv("C:/Users/AllanaNunes/Desktop/Python/bd_jan21.csv",h=T, sep=";", dec=",")
## Manipulações
...
```{r}
individual_prod<-bd %>% group_by( Barco,SPP, Municipio, Origem,Ano) %>%
  summarize(Receita = sum(Receita, na.rm=T)) # barcos por entreposto por 

names(individual_prod)<-c("Barco", "SPP", "Municipio", "Comunidade", "Ano", "Receita")

cadastroX<-merge(x=cadastro1, y=individual_prod,by=c("Barco", "Comunidade","Municipio", "SPP" ),all.x=T, all.y=F)


plotZ<-function(i) {ggplot(cadastroX, aes(x=Ano, y=Receita))+
geom_bar(stat="identity", color="black", fill=receita_cor)+
theme_minimal()}
plotZ
...
leaflet() %>%
  addTiles() %>%
  addMarkers(data=cadastro1, lat=cadastro1$Latitude, lng=cadastro1$Longitude, group = "points")%>%
   addPopupGraphs(list(), group = "pnt", width = 300, height = 400)
